...
//		DEBUG_printf("BMP180 error\n", NULL);											// Device error
	...
	if((pdTRUE != xTaskCreate(vBlinker,"Blinker", 32, &blinkParam, tskIDLE_PRIORITY + 1, &xTaskHandleBlinker))) {
...
//	}
	...
	if((pdTRUE != xTaskCreate(vESP8266Task,"ESP_Main", 512, NULL, tskIDLE_PRIORITY, &xTaskHandleESP))) {
		ERROR_ACTION_CRITICAL();
	}

	if((pdTRUE != xTaskCreate(vBMP180_sample,"BMP180", 128, NULL, tskIDLE_PRIORITY + 1, &xTaskHandleBMP180Sample))) {
		ERROR_ACTION_CRITICAL();
	}

	if((pdTRUE != xTaskCreate(vMAX44009_sample,"MAX44009", 64, NULL, tskIDLE_PRIORITY + 1, &xTaskHandleMAX44009Sample))) {
		ERROR_ACTION_CRITICAL();
	}

	if((pdTRUE != xTaskCreate(vSHT11_sample,"SHT11", 64, NULL, tskIDLE_PRIORITY + 1, &xTaskHandleSHT11Sample))) {
		ERROR_ACTION_CRITICAL();
	}

	if((pdTRUE != xTaskCreate(vUSART_debug,"debug", 64, NULL, tskIDLE_PRIORITY + 1, &xTaskHandleDebug))) {
		ERROR_ACTION_CRITICAL();
	}
