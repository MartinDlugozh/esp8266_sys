...
void ST7735_Init(void) {
	...
	vTaskDelay(120); // Datasheet says what display wakes about 120ms (may be much faster actually)
...
void ST7735_PutChar5x7(uint16_t X, uint16_t Y, uint8_t chr, uint16_t char_color, uint16_t background_color)
{
	uint16_t i,j;
	uint8_t buffer[5];
    uint8_t C_CH = char_color >> 8;
    uint8_t C_CL = (uint8_t)char_color;
    uint8_t B_CH = background_color >> 8;
    uint8_t B_CL = (uint8_t)background_color;
...
void ST7735_PutStr5x7(uint8_t X, uint8_t Y, char *str, uint16_t str_color, uint16_t background_color)
{
    while (*str) {
        ST7735_PutChar5x7(X,Y,*str++,str_color, background_color);
...
void ST7735_fillCircle(uint8_t x0, uint8_t y0, uint8_t r, uint16_t color)
{
  ...
  while (x<y) {
    ...
    ST7735_VLine(x0-x, y0-y, 2*y+1, color);
    ST7735_VLine(x0+y, y0-x, 2*x+1, color);
    ST7735_VLine(x0-y, y0-x, 2*x+1, color);
  }
}
